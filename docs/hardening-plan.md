...
The baseline had 116 passing tests. The local integrated suite has 247 passing tests and six Windows-only cases exercised by CI. A fresh wheel passed the installed-package check outside the repository. Browser checks observed live data, a visible warning after malformed UDP, disconnection after server shutdown, and automatic reconnection with old rows preserved. The gstack browser lacked its Chromium runtime, so those UI checks used the built-in browser.
